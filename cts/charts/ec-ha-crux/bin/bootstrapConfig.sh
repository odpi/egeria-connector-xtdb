...
# Setup one configuration that we will use for all instances of the same server
...
# Deploy this configuration to each pod running the server
...
# Start each pod running the server
for REPLICA in $(seq 0 ${NUM_REPLICAS}); do
  ENDPOINT="https://${CRUX_RELEASE_NAME}-crux-${REPLICA}.${INTERNAL_SVC_NAME}:9443"
